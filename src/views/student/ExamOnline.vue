    ...
    <el-main>
      ...
      <el-table
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button size="small" :disabled="!checkExam(scope.row) || scope.row.flag" @click="toStartExam(scope.row)"
                     
              :icon="checkExam(scope.row) ? (scope.row.flag ? 'el-icon-check' : 'el-icon-edit') : 'el-icon-lock'"
              :type="checkExam(scope.row) ? (scope.row.flag ? 'success' : 'primary') : 'info'">
              {{ checkExam(scope.row) ? (scope.row.flag ? '已结束' : '去考试') : '暂不开放' }}
            </el-button>
          </template>
        </el-table-column>
  ...
  computed: {
    ...
    checkExam (row) {
      return (row) => {
        ...
        } else if (date >= row.startTime && date <= row.endTime) return true
      }
    }
  }
}
